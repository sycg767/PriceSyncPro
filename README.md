...
- **🔄 批量更新**：一键批量更新所有渠道的价格配置，智能检测API类型，自动适配New API和One Hub。
- **📝 智能输入**：URL和前缀输入自动优化，支持下拉选择API路径，前缀自动补全 `/`。
- ** 更安全**：通过浏览器 Cookie 进行认证，无需暴露和管理 API Token。
- **🎯 更便捷**：直接在 New API 后台页面使用，无需切换任何外部工具。
- **💾 配置预设**：保存并管理多个上游渠道的配置，一键加载，轻松切换。
- **📊 实时反馈**：清晰的 UI 反馈和进度条，让每一步操作的状态都一目了然。
...
1.  **切换到"自动配置"模式**。
2.  **填写上游信息**:
    *   **上游基础 URL**: 上游的基础地址 (例如: `https://api.example.com`)
    *   **API 路径**: 从下拉框选择或自定义 (例如: `/api/pricing`)
    *   **渠道前缀**: 你想为这批模型添加的前缀 (例如: `Silo`，无需添加 `/`)
...
2.  **填写基础配置**:
    *   **上游基础 URL**: 上游的基础地址
    *   **API 路径**: 从下拉框选择（支持 New API、One Hub 或自定义）
    *   **渠道前缀** (可选): 如果你的模型有前缀，请填写（无需添加 `/`）
...
2.  确认操作后，插件会自动：
    *   获取所有渠道的 `base_url`
    *   智能尝试 `/api/pricing` 和 `/api/available_model` 两种API路径
    *   逐个更新每个渠道的价格配置
    *   显示实时进度和详细的成功/失败统计
3.  完成后会展示完整的结果报告，包括所有失败渠道的详细错误信息。
